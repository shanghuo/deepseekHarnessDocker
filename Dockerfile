...
RUN mkdir -p /etc/dsh \
 && printf '%s\n' \
    ...
    '' \
    '## Bun 开发规范（必读）' \
    '- **文件处理必须使用 Bun 提供的 node 兼容 API（如 node:fs、node:path），禁止使用 Bun 自有的文件相关 API**（Bun.file / Bun.write / Bun.read 等）' \
    '  - 原因: Bun 自有文件 API 存在不可控的缓存/内存泄漏' \
    '- Bun 的其它 API（网络、运行时等）不做限制' \
    '- 可能的大文件优先使用流式处理（streams），避免整体载入内存' \
    '' \
    '## 网络安全约定' \
